...
## Company-wise saved formats (2026-06 fork)
- User wanted to SAVE their arranged layout per company and have future scans of that company auto-use it. Backend: `company_formats` collection (store-scoped, one per company via upsert). Endpoints: `POST /api/company-formats {company, template}` (upsert), `GET /api/company-formats` (list), `DELETE /api/company-formats/{company}`. Verified via curl (save + upsert keeps 1 doc per company).
- Format template stores only the ARRANGEMENT recipe: per-line {text, x, y, size, bold}, code {type, sizeMm, box.y}, logo {box} — not scan-specific values.
- Frontend scan-sticker: new "Save as {company} FORMAT" button (`saveCompanyFormat`). On mount, `loadSaved` also fetches all company formats into `companyFormats` map. On scan (processImage) and company-switch (applyCompany), if a saved format exists it is re-applied via `applyCompanyFormat`: lines matched by `fieldKey` (text before ':' with digits stripped — so recurring OEM fields like "HKMC P/N", "MODEL", "IFT ID" land where the user placed them), plus code type/size and logo box. Unmatched lines keep their auto position. Toast shows "(Company saved format)" when applied.
